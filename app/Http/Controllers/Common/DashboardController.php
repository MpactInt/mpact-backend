...
class DashboardController extends Controller
{
     ...
     public function getDailyVisitData(Request $request)
    {
        ...
        $sql = "SELECT count(DISTINCT(activity_logs.user_id)) user_count, LEFT(DAYNAME(login_time), 3) visit_day, MAX(activity_logs.login_time) AS last_login_time FROM activity_logs JOIN company_employees ON activity_logs.user_id = company_employees.user_id WHERE company_employees.company_id = ".$company->id." and DATE(login_time) >= CURDATE() - INTERVAL 7 DAY group by visit_day ORDER BY last_login_time";
